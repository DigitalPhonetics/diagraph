...
def _serialize(key: str, value: Any) -> Dict[str, Any]:
    if isinstance(value, dict):
        if len(value) == 0:
            return {key: {}}
        return {key: reduce(lambda d1, d2: d1 | d2, [_serialize(inner_key, value[inner_key]) for inner_key in value], {})}
...
class _ServiceFunctionDelegate:
    ...
    def publish(self, other, topic: str, value: Any, user_id: int):
        print("PUBLISHING RESULT TO", topic + self.domain_suffix + "." + user_id)
        serialized = _serialize(topic, value)       
        serialized["user_id"] = user_id
        other._component._session.publish(topic + self.domain_suffix + "." + user_id, **serialized)
        ...
        for func_name in dir(self):
            ...
            if hasattr(func_inst, "_pubsub"):
                ...
                for topic in set(delegate.sub_topics.keys()).union(delegate.sub_topics_queued.keys()):
                    res = session.subscribe(partial(delegate.receive, other=self), topic=topic, options=SubscribeOptions(match="prefix"))
            ...
            if result:
                ...
                for pub_topic in pub_topics:
                    if pub_topic in result:
                        ...
                        serialized["user_id"] = user_id
                        self._component._session.publish(pub_topic + self._domain_suffix + "." + user_id, **serialized) 
