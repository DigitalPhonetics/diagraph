...
class DialogSystem:
    ...
    async def _on_dialog_state_changed(self, user_id: int, **kwargs):
        ...
        #     if kwargs[key] == False:
        #         return
        print("DS STATE CHANGED FOR USER", user_id)
    ...
    def run(self, start_messages: Dict[str, Any] = {ControlChannelMessages.DIALOG_START: True}, user_id: int = 0):
        """
        ...
        tasks = []
        if start_messages:
            tasks.append(self._on_dialog_state_changed(user_id=user_id, changed=True))
            tasks += [self._publish(msg, start_messages[msg]) for msg in start_messages]
    ...
    async def _onJoin(self, session, details):
        ...
        self._ctrl_component._session.subscribe(self._on_dialog_state_changed, topic=ControlChannelMessages.DIALOG_START, options=SubscribeOptions(match="prefix"))
        self._ctrl_component._session.subscribe(self._on_dialog_state_changed, topic=ControlChannelMessages.DIALOG_END, options=SubscribeOptions(match="prefix"))
